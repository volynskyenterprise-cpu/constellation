...
`subject_alternates` is the preferred alternate-evidence schema. Comparable Intelligence also reads document alternates under `subject_evidence.alternates`, direct field assertions under `subject_evidence.<field>.alternate_values`, and prior nested assertions under `subject_evidence.fields.<field>.alternate_values`. Scalar, mapping, and documented field-keyed alternates inherit only explicit source metadata from the alternate, field, and enclosing evidence document. Exact duplicate assertions are consolidated with their schema paths retained; different-source assertions remain distinct. Descriptive prose is preserved but never converted into unit, accessory-unit, bedroom, bathroom, or other structured facts.

v7.3.6 treats empty alternate collections—including the local YAML reader's scalar `"[]"` representation—as valid empty evidence rather than malformed input. Provenance inherits deterministically from alternate, field, then document scope; one parent path may populate the compatible singular path, while multiple parent paths remain preserved without an arbitrary choice or false missing-provenance review. Assignment-specific Markdown reports show concise source, verification, schema-origin, and schema-path details. Byte-identical review-state output is not rewritten, so a no-op build preserves its modification time and existing history behavior.
